          ...
          <Fragment key={timeSlotId}>
            <Collapsible>
              <div className="flex flex-row items-center gap-2">
                <H4>{todos.find(t => t.timeSlot.id === Number.parseInt(timeSlotId, 10))?.timeSlot.name}</H4>
                <Large>{todos.find(t => t.timeSlot.id === Number.parseInt(timeSlotId, 10))?.timeSlot.description}</Large>
              ...
              <CollapsibleContent>
                {todos.filter(t => t.timeSlot.id === Number.parseInt(timeSlotId, 10)).map(todo => (
                  <div key={todo.todoItem.id} className="flex flex-col gap-1">
